...
def Send():
    ...
    def sender():
        ...
        file_data=file.read(1024000000)
        conn.send(file_data)
        print("Data has been transmitted successfully..")

    #icon
    image_icon1=PhotoImage(file="Image/send2.png")
    window.iconphoto (False,image_icon1)

    Sbackground=PhotoImage(file="Image/sender.png")
    Label(window,image=Sbackground).place(x=-2,y=0)

    Mbackground=PhotoImage(file="Image/mAdrModify.png")
    Label (window, image=Mbackground,bg = '#f4fdfe').place(x=100, y=270)

    host=socket.gethostname()
    Label (window, text=f'ID: {host}',bg='white',fg='black').place(x=170, y=385)

    Button(window,text="+ select file",width=10,height=1,font='sans 14 bold',bg="#fff",fg="#000",command = select_file).place(x=160,y=170)
    Button(window, text="SEND",width=8, height=1, font='sans 14 bold', bg='#000',fg="#fff",command = sender).place(x=300, y=170)
...
def Receive():
    ...
    Label (main, image=Hbackground).place(x=-2)

    logo=PhotoImage(file='Image/mushroom3.png')
